...
這個 Cloudflare Worker 是 GitHub Pages 與 OpenAI Responses API 之間的安全 proxy。`OPENAI_API_KEY` 只存為 Cloudflare secret，不得放入網站 JavaScript、`wrangler.toml` 或 Git。
...
2. 在本資料夾執行 `wrangler secret put OPENAI_API_KEY`，將本機 `.env.local` 的值安全存成 Worker secret。
