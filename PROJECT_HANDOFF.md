...
- 最後更新：2026-07-24
- 交接版本：v20（獨立 Supabase project、RLS 與 Magic Link 網址已完成連接）
- 本機 repo：`/Users/tinrepin/Desktop/medical`
- GitHub repo：`dale199707/schwartz-notes`（private／closed-source）
- 目前整合 commit：`197a231`（`Add private Zollinger surgical figures`）
- 預設整合分支：`main`
- 存取原則：repository 必須保持 private；不得假設舊 GitHub Pages 網址仍是正式交付面，也不得把私人原書圖版重新公開

目前功能分支：`codex/account-sync`。使用者於 2026-07-24 指示先製作登入同步功能，現階段網站可維持 public 閱讀，暫不把 private access gate 納入本批；這只改變本批開發優先順序，不代表 Zollinger 原書圖版的著作權限制失效。
...
- `codex/account-sync` 已加入 public-read／optional-login 架構：未登入維持 localStorage，登入後以 Supabase Email Magic Link、Postgres RLS 與離線 outbox 同步個人筆記、★及螢光標記。獨立 Supabase project `medical-notes` 已建立於 Singapore，migration、RLS、正式 Site URL／Redirect URL 與前端 publishable 設定均已完成；功能分支尚未 commit、push 或部署，實際 Email Magic Link 與雙裝置端對端測試仍待正式網址更新後執行。
...
此專案是無框架的靜態網站，沒有 npm build step；過去由 GitHub Pages 發布，目前 repository 為 private，是否部署及部署方式必須由使用者另行決定。
