...
- 最後更新：2026-07-24
- 交接版本：v21（登入同步已合併 `main` 並部署至 GitHub Pages）
- 本機 repo：`/Users/tinrepin/Desktop/medical`
- GitHub repo：`dale199707/schwartz-notes`（本次部署期間依使用者指示暫時 public；驗收後須立即改回 private／closed-source）
- 目前功能整合 commit：`e7f1d7d`（`Harden sync client availability checks`；後續交接文件 commit 以 Git 為準）
- 預設整合分支：`main`
- 存取原則：本次公開只為完成使用者明確要求的部署與驗收；repository 內含私人原書圖版，驗收後須由使用者立即改回 private，不得把暫時 public 視為持續公開授權

登入同步功能已由 `codex/account-sync` fast-forward 合併至 `main`。使用者於 2026-07-24 明確要求先維持 public 完成本次 push／GitHub Pages 部署，待本次操作完成後再自行改回 private；這不代表 Zollinger 原書圖版的著作權限制失效。

## 0. 給新 Chat 的最短指令

新 Chat 開始時，使用者可直接貼上：

> 請先完整閱讀 `/Users/tinrepin/Desktop/medical/PROJECT_HANDOFF.md`、專案 `/Users/tinrepin/Desktop/medical/README.md` 與 `/Users/tinrepin/Downloads/AGENTS.md`，確認實際 repo 是 `/Users/tinrepin/Desktop/medical`，並檢查 branch、`git status`、`main`／`origin/main` 與最新 commit。此 repository 已是 private／closed-source，內含 Zollinger 私人原書圖版，不得改回 public 或部署到公開 Pages。接著協助新增第四本書：保留 Schwartz、ICU Book 與 Zollinger，不大幅重寫 `main` 或既有 loader；從乾淨 `main` 建立 `codex/<book-id>-library`，先完成新書網站骨架、正式目錄、獨立資料夾與 Claude 交稿規格，不要先自行撰寫整本內容。Claude 原稿只放新書的 `claude/`；Codex 之後再做格式整理、來源查核、十題 evidence audit、同步 `chapters/`、網站驗證與分批提交。沒有使用者明確指示，不得 merge `main`、push 私人教材圖片或部署任何網站。
...
新 Chat 讀完本檔後，若上述資料已齊全，不需重問；直接檢查教材與 TOC、提出最小變更範圍並建立新書骨架。若缺少會影響正式目錄、chapter count、檔名或圖片權利範圍的資訊，才向使用者確認。若使用者只說「想做新書」，先收集本表，不要猜書名、版本或章數。
...
- `main` 與 `origin/main` 同步於 `197a231`（`Add private Zollinger surgical figures`）。
- Repository 已由使用者改為 private／closed-source。
...
- `main` 已加入 public-read／optional-login 架構：未登入維持 localStorage，登入後以 Supabase Email Magic Link、Postgres RLS 與離線 outbox 同步個人筆記、★及螢光標記。獨立 Supabase project `medical-notes` 已建立於 Singapore，migration、RLS、正式 Site URL／Redirect URL 與前端 publishable 設定均已完成；功能已 push 並部署至 `https://dale199707.github.io/schwartz-notes/`。正式 HTML、`account-sync.js`、`sync-config.js` 已核對，登入視窗可顯示且瀏覽器無 console error；實際 Email Magic Link 與雙裝置端對端測試仍待使用者以自己的信箱執行。

接手時仍須重新以 Git 確認，不可只相信本段日期。除非使用者明確要求部署，否則不要探測、重啟或重新發布舊 GitHub Pages。
...
- 狀態：正式內容完成，Chapter 1–150 為 `ready` 且 150 / 150 `passed`，書籍 metadata 為 `complete`。全書內容與私人圖片已整合至 commit `197a231`。

Zollinger 的原書手術圖可由工具從 PDF 轉成頁面 PNG 或抽出內嵌 JPEG；本機 `Zollinger_Images/` 已包含按 chapter 分類的裁切圖版與 `_INDEX.csv`，僅可供私人定位與理解。McGraw-Hill 版權頁禁止未經授權重製或散布，因此不得提交到公開 repository 或放上公開 Pages；可散布版本只使用 Public Domain、相容 Creative Commons、已取得授權或未仿製教材構圖的原創圖解。
...
此專案是無框架的靜態網站，沒有 npm build step；本次已依使用者明確要求由 `main` 發布至 GitHub Pages。repository 只在本次驗收期間暫時 public，完成後須改回 private；日後是否部署及部署方式仍必須由使用者另行決定。
...
- `books/zollinger-10e/chapters.js`：Zollinger 章節目錄、Markdown fetch、parser 與 notes registry。
- `books/zollinger-10e/figures.js`：可散布的原創／合法授權圖片。
- `books/zollinger-10e/private-figures.js` 與 `private-figures/`：只限私人閉源環境使用的 Zollinger 原書圖版。
...
完成後必須回歸測試 Schwartz、ICU 與 Zollinger，確認內容、搜尋、章節數與瀏覽器保存資料均未互相污染。
...
章號檔名需使用固定寬度，確保字典序與章序一致：少於 100 章可使用兩位數；100 章以上優先使用三位數。新書一旦選定寬度，`claude/`、`chapters/`、`audits/` 與 loader 必須一致，不可中途混用。
...
`codex/account-sync` 新增：

- `sync-config.js`：只放 `medical-notes` Supabase project URL、browser publishable key 與 table name；不得加入 secret／service role key。
- `account-sync-core.js`：stable JSON fingerprint、★聯集、螢光範圍合併及同章筆記衝突保留。
- `account-sync.js`：Email Magic Link、session、離線 outbox、首次本機資料遷移、remote-first 衝突比對、focus／online retry、登出資料隔離與多帳號本機 snapshot。
- `supabase/migrations/202607240001_medical_note_state.sql`：`medical_note_state`、RLS policies、updated_at trigger、最小權限、正式書籍／章節白名單與 payload 類型／大小限制。
- `scripts/test-account-sync.mjs`、`scripts/test-account-sync-integration.mjs`：同步核心與 mock Supabase 整合回歸測試。

雲端 row 的識別欄位為 `user_id + book_id + chapter_id + kind`，其中 `kind` 僅允許 `personal-notes`、`important`、`highlights`。前端只使用 publishable key 與使用者 session；service role key 絕不可出現在 repository。

同步規則：

1. 未登入仍立即寫入既有 localStorage。
2. 登入後每次變更先寫本機及 outbox，再 debounce 上傳。
3. 視窗重新取得焦點或恢復網路時自動 pull／retry。
4. 同步時先讀 remote，再以最後一次共同 fingerprint 判斷單邊修改或雙邊衝突，完成合併後才寫回；不得先把 outbox 覆蓋 remote。
5. 初次登入若 remote 無該 record，以上傳本機資料為主；若 local 無該 record，以下載 remote 為主。
6. 無共同 sync fingerprint 且雙方不同時：★取 union，highlight 以本機選取範圍覆蓋重疊遠端範圍，文字筆記以分隔線保留兩版本。
7. 登出時封存該帳號 snapshot 並切回獨立的未登入空間；同一瀏覽器切換帳號時先保存前一帳號 snapshot，再載入新帳號 snapshot，避免畫面洩漏或把前一帳號資料上傳給新帳號。
8. 同步 request 進行期間若發生登出／帳號切換，舊 request 在套用本機資料前會核對 user id 並中止。

2026-07-24 後端連接狀態：

1. 已建立獨立 Supabase project `medical-notes`，region 為 Southeast Asia (Singapore)。
2. 已執行 migration，建立 `medical_note_state`、updated-at trigger、四條 authenticated-only RLS policies，以及正式書籍／章節、payload shape 與 100,000-byte 單筆上限 constraint。
3. 已將正式 Site URL 與 Redirect URL 設為 `https://dale199707.github.io/schwartz-notes/`。
4. 已將 project URL 與 publishable key 填入 `sync-config.js`；repository 未包含 secret／service role key。
5. `test-account-sync.mjs`、`test-account-sync-integration.mjs`、三個 JavaScript syntax check 與 localhost UI 載入均通過；整合測試涵蓋雙裝置文字衝突、登出隔離、第二帳號接手未登入資料與 in-flight sync 登出競態，瀏覽器 console 無 error。
6. 使用 publishable key 直接以未登入角色讀取 `medical_note_state` 會收到 HTTP 401／permission denied，確認 anon 無資料表讀取權。

正式部署後仍需以兩個瀏覽器 session 實測 Email Magic Link、初次匯入、跨裝置 pull、衝突、登出及帳號切換；未完成這項端對端驗證前，不宣稱跨裝置同步已正式通過。
...
2. 在 `books.js` 登錄新書。
3. 依現有 `window.MEDICAL_BOOK_DATA[bookId]` registry 接入新書 loader，不要為第四本書另建互不相容的載入系統。
4. 將新書 script／pending loader 接入頁面。
5. 驗證 selector、章節目錄、placeholder、搜尋、AI context 與 localStorage isolation。
6. 回歸測試 Schwartz／ICU／Zollinger。
...
### Phase E：私人 Git 整合與選擇性部署

1. 確認功能分支乾淨且 tests 通過。
2. 同步最新 `main`，避免覆蓋遠端更新。
3. 使用者明確同意後才 push 功能分支及 fast-forward／merge 到 private `main`。
4. Push 到 private repository 不等同授權公開部署；預設只完成 Git 備份與整合。
5. 若使用者要求線上閱讀，先明確選擇：local-only、具登入／存取控制的私人 hosting，或只含合法可散布內容的 public site。
6. 含 `private-figures/` 或其他私人教材內容的版本不得部署到公開 Pages。
7. 若部署的是可公開版本，才等待 deployment 成功並用 cache-busting query 驗證 HTML、loader 與 Markdown assets。
...
14. Push 後確認遠端 branch SHA；只有實際執行部署時才檢查 deployment 與正式 HTML。

因 ICU、Zollinger 與未來新書會在瀏覽器以 `fetch()` 載入 Markdown，本機完整測試宜使用 local static server；單純 `file://` 可能被瀏覽器阻擋。若使用者另行授權部署，再依選定的 private hosting 或可公開版本驗證正式網址。
...
- 未經使用者明確同意，不合併 `main`、不部署網站。
- Repository 必須保持 private；任何要求改為 public、建立公開 Pages 或分享私人教材圖片，都必須先停下來確認。
- 文件或網站發布後，需回報 branch、commit、push；若沒有部署，明確寫「未部署」，不要把 push 說成 deployment。

## 17. 不可踩的坑

1. 不要把 `claude/` 直接當網站來源。
2. 不要因內容看起來完整就跳過十題 audit。
3. 不要只在 `books.js` 加新書而忘記建立該書自己的 registry loader。
4. 不要讓新書 chapter 1 覆蓋其他書 chapter 1 的 localStorage。
...
Schwartz、ICU Book 與 Zollinger 已全部完成；下一個工作階段不需再驗證或重做這三本書。新 Chat 應依下列順序開始：

1. 讀完本交接檔、`README.md`、`AGENTS.md`，確認 `main`／`origin/main` 與工作目錄乾淨。
2. 向使用者取得第 0 節資料表；若資料已在對話或資料夾中，直接讀取，不重複詢問。
3. 檢查新書 PDF／TOC／圖片資料夾，只做唯讀盤點：書名、edition、年份、作者、章數、sections、頁碼與檔案結構。
4. 提出 `<book-id>`、固定寬度 chapter filename、每批章數與圖片策略。
5. 從乾淨 `main` 建立 `codex/<book-id>-library`。
6. 第一階段只建立獨立 `books/<book-id>/` 骨架、完整正式目錄、loader、selector、storage isolation、Claude 指令與 audit index；除非使用者要求，不先撰寫整本內容。
7. 在本機 static server 驗證新書 placeholder、搜尋、AI context 與 browser storage；回歸確認 Schwartz 54/54、ICU 53/53、Zollinger 150/150。
8. 骨架完成後交付一段可直接貼給 Claude 的寫作指令，要求 Claude 只寫新書 `claude/`。
9. Claude 每批交稿後，由 Codex 使用 `validate-medical-chapter` skill 做十題 evidence audit，修訂並同步至 `chapters/`。
10. Commit／push／merge／部署都等待使用者明確指示；新書圖片若不是 Public Domain、相容 CC、已授權或使用者明確限定的私人資料，不得加入。

### 第四本書開始前不可破壞的既有基準

- Schwartz：54 / 54 `passed`。
- ICU Book：53 / 53 `passed`。
- Zollinger：150 / 150 `passed`。
- Zollinger 私人圖版：260 張、144 個章節；Chapter 5 兩張、Chapter 88 九張、Chapter 150 一張；Chapter 1–4、93、96 無圖。
- `main` 功能基準：`e7f1d7d`；後續若只有交接文件 commit，以實際 Git HEAD 為準。
- Repository：本次部署期間暫時 public；驗收後必須恢復 private／closed-source。
- 原始 PDF、`Zollinger/Zollinger_Images/`、`.DS_Store`、secrets 不得追蹤。
